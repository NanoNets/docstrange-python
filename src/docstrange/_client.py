...
from ._exceptions import APIStatusError, DocstrangeError
...
if TYPE_CHECKING:
    from .resources import api, chat
...
class Docstrange(SyncAPIClient):
    # client options
    api_key: str
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new synchronous Docstrange client instance.

        This automatically infers the `api_key` argument from the `DOCSTRANGE_API_KEY` environment variable if it is not provided.
        """
        if api_key is None:
            api_key = os.environ.get("DOCSTRANGE_API_KEY")
        if api_key is None:
            raise DocstrangeError(
                "The api_key client option must be set either by passing api_key to the client or by setting the DOCSTRANGE_API_KEY environment variable"
            )
        self.api_key = api_key

        if base_url is None:
            base_url = os.environ.get("DOCSTRANGE_BASE_URL")
        if base_url is None:
            base_url = f"https://extraction-api.nanonets.com"
    ...
    @cached_property
    def with_streaming_response(self) -> DocstrangeWithStreamedResponse:
        return DocstrangeWithStreamedResponse(self)
...
class AsyncDocstrange(AsyncAPIClient):
    # client options
    api_key: str
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new async AsyncDocstrange client instance.
    ...
    @cached_property
    def with_streaming_response(self) -> AsyncDocstrangeWithStreamedResponse:
        return AsyncDocstrangeWithStreamedResponse(self)
...
class DocstrangeWithRawResponse:
    _client: Docstrange

    def __init__(self, client: Docstrange) -> None:
        self._client = client
...
class AsyncDocstrangeWithRawResponse:
    ...
    def __init__(self, client: AsyncDocstrange) -> None:
        self._client = client
